...
function renderPlayerModal(playerId){
  ...
  /* Only the categories this player actually picked. A game they skipped
     entirely drops out of the list rather than showing six empty rows. */
  var rows = games.map(function(g){
    ...
    var made = [];
    function add(label, html){
      made.push('<div class="pd-row"><span>' + label + '</span>' + html + '</div>');
    }
    if (picks.spread)        add('Spread', pickLabel(picks, "spread", g.away, g.home, lines.awayLine, lines.homeLine, gr));
    if (picks.total)         add('Total', ouLabel(picks, "total", lines.total, gr));
    if (picks.h1Spread)      add('1H Spread', pickLabel(picks, "h1Spread", g.away, g.home, lines.h1AwayLine, lines.h1HomeLine, gr));
    if (picks.h1Total)       add('1H Total', ouLabel(picks, "h1Total", lines.h1Total, gr));
    if (picks.teamTotalAway) add(escapeHtml(awayT) + ' Total', ouLabel(picks, "teamTotalAway", lines.awayTeamTotal, gr));
    if (picks.teamTotalHome) add(escapeHtml(homeT) + ' Total', ouLabel(picks, "teamTotalHome", lines.homeTeamTotal, gr));
  ...
  var summary = graded
    ? '<div class="pd-summary">Week ' + (STATE.week || 1) + ': <b>' +
        fmtRecord(card.picksRecord) + '</b> <span>(' + fmtPct(winPct(card.picksRecord)) + ')</span>' +
      '<br>Mortal <b>' + fmtRecord(card.mortalRecord) + '</b> · Super Dog <b>' + fmtPts(card.superDog) +
        ' pts</b> · <b>' + fmtPts(card.total) + ' pts</b> total' +
      (season && season.weeks
        ? '<br>Season: <b>' + fmtRecord(season.picks) + '</b> <span>(' + fmtPct(winPct(season.picks)) +
          ')</span> · ' + fmtPts(season.total) + ' pts'
        : '') + '</div>'
    : '<div class="pd-summary">Week ' + (STATE.week || 1) + ' — not graded yet' +
      (season && season.weeks
        ? '<br>Season so far: <b>' + fmtRecord(season.picks) + '</b> <span>(' +
          fmtPct(winPct(season.picks)) + ')</span>'
        : '') + '</div>';
  ...
  '<div class="modal-backdrop" id="pd-backdrop">' +
    '<div class="modal-sheet">' +
      ...
      '</div>' +
      '<div class="pd-games">' +
        (rows || '<div class="empty">No picks made this week.</div>') +
      '</div>' +
    '</div>' +
  '</div>';
}
...
/* The grey line under a player's name: their standard-pick record, then
   Mortal's own record, then Super Dog's points. Only the first of those
   drives the ranking. */
function standingLine(picksRec, mortalRec, dogPts){
  var moPct = winPct(mortalRec);
  return fmtRecord(picksRec) + ' picks' +
    ' \u00b7 Mortal ' + fmtRecord(mortalRec) +
    (moPct === null ? '' : ' (' + fmtPct(moPct) + ')') +
    ' \u00b7 Dog ' + fmtPts(dogPts) + ' pts';
}

/* One standings row. `right` is the headline value on the right-hand side \u2014
   the win percentage once a week is graded, or the pick count before that. */
function standingRow(i, id, name, line, right){
  return '<button type="button" class="board-row" data-player-id="' + escapeHtml(id) + '">' +
    '<span class="board-rank">' + (i+1) + '</span>' +
    '<span class="board-id">' +
      '<span class="board-name">' + escapeHtml(name) + '</span>' +
      '<span class="board-line">' + line + '</span>' +
    '</span>' +
...
function renderSeasonBoard(){
  ...
  var rows = totals.map(function(t, i){
    return standingRow(i, t.id, t.name,
      standingLine(t.picks, t.mortalRec, t.superDog),
      fmtPct(winPct(t.picks)));
  }).join("");
  var breakdown = totals.map(function(t){
    return '<div class="board-mini">' +
      '<span class="n">' + escapeHtml(t.name) + '</span>' +
      '<span class="v">' + fmtPts(t.total) + ' pts total \u00b7 ' +
        fmtPts(t.standard) + ' picks \u00b7 ' + fmtPts(t.superDog) + ' dog \u00b7 ' +
        fmtPts(t.mortal) + ' mortal</span>' +
    '</div>';
  }).join("");
  ...
  '<div class="board">' +
    ...
    '<p class="board-note">Season standings across ' + played + ' graded week' + (played === 1 ? '' : 's') +
      '. Ranked by win percentage on standard picks \u2014 a push counts as half a win. ' +
...
function renderWeekBoard(){
  ...
  var list = Object.keys(entries).map(function(id){
    var e = entries[id];
    var card = graded ? gradeEntry(games, results, e) : null;
    return {
      id: id,
      name: e.name || "Anonymous",
      made: scoreOf(e),
      card: card,
      /* Shaped like a season standing so one comparator serves both boards. */
      picks: card ? card.picksRecord : blankRecord(),
      mortalRec: card ? card.mortalRecord : blankRecord(),
      total: card ? card.total : 0,
      superDog: e.superDog || null,
      mortal: e.mortal || null
    };
  }).filter(function(e){ return e.name; });

  if (graded) list.sort(compareStandings);
  else list.sort(function(a,b){ return b.made - a.made; });

  var totalCats = games.length * CATEGORIES.length;
  var rows = list.map(function(e, i){
    if (!graded){
      /* Before kickoff the count is already on the right, so the line below
         the name reports the two weekly specials instead of repeating it. */
      return standingRow(i, e.id, e.name,
        (e.superDog ? 'Super Dog set' : 'No Super Dog yet') + ' · ' +
        (e.mortal ? 'Mortal set' : 'No Mortal yet'),
        e.made + ' / ' + totalCats);
    }
    return standingRow(i, e.id, e.name,
      standingLine(e.picks, e.mortalRec, e.card.superDog),
      fmtPct(winPct(e.picks)));
  }).join("");
  ...
  var note = graded
    ? 'Week ' + (STATE.week || 1) + ' final. Records are wins\u2013losses\u2013ties on standard picks, ' +
      'and the percentage treats a tie as half a win. Tap a name for the full card.'
    : 'Records appear once games finish. Until then this shows how many picks each player has ' +
      'locked in \u2014 tap a name to see them.';
